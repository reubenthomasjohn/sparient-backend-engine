...
export class SyncOrchestrator {
  ...
  async syncFile(
    ...
    const { batchId, wasAlreadyInFlight } = await batchBuilder.buildForFile(
      institution,
      course,
      postUpload,
      { isInitialSync: false, forceReprocess: force, s3Bucket },
    );
    ...
    return { batchId, sourceFileId: postUpload.id, wasAlreadyInFlight };
  }
}
