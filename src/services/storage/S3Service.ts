...
class S3Service {
  ...
  async putJson(bucket: string, prefix: string, key: string, body: unknown): Promise<void> {
    ...
    logger.info('S3: json written', { bucket, key: fullKey });
  }
  ...
  async fileExists(bucket: string, prefix: string, key: string): Promise<boolean> {
    const fullKey = `${prefix}/${key}`;
    try {
      await this.client.send(new HeadObjectCommand({ Bucket: bucket, Key: fullKey }));
      return true;
    } catch {
      return false;
    }
